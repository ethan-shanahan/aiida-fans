...
def input_validator(input:str, note:str) -> function: # type: ignore  # noqa: F821
    match input:
        case "filename":
...
def matmodel_validator(x:Str, note:str) -> str | None:
    valid = {"LinearThermalIsotropic", "LinearElasticIsotropic", "PseudoPlasticLinearHardening", "PseudoPlasticNonLinearHardening", "J2ViscoPlastic_LinearIsotropicHardening", "J2ViscoPlastic_NonLinearIsotropicHardening"}
    return note if x not in valid else None
...
class FANSCalculation(CalcJob):
    ...
    def define(cls, spec: CalcJobProcessSpec) -> None:
        ...
        # spec.input.microstructure((input:="filename"),  valid_type=Str, validator=input_validator(input, note), help=(note:="This specifies the path to the HDF5 file that contains the microstructure data."))
        ...
        spec.input("matmodel",                      valid_type=Str,         validator=lambda x: matmodel_validator(x, note),                            help=(note:="This specifies the material model to be used in the simulation."))
